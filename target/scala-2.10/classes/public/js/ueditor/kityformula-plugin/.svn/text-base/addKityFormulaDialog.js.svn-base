UE.registerUI('kityformula', function(editor, uiname){
    ...
    var kfDialog = new UE.ui.Dialog({
        ...
        // 指定弹出层路径
        iframeUrl: editor.options.UEDITOR_HOME_URL + 'kityformula-plugin/kityFormulaDialog.html?examId='+$("#examinationPaperId").val()+'&cursorDivId='+$("#cursorDivId").val()+'&range='+$("#cursor").val()+'&questionId='+$("#cursor_questionId").val()+"&type="+$("#cursor_type").val() ,
        // 编辑器实例
        editor: editor,
        // dialog 名称
        name: uiname,
        // dialog 标题
        title: '插入公式 - SmartClass',
        	        ...
        	        buttons:[
        	            {
        	                ...
        	                onclick:function () {
        	                    kfDialog.close(true);
        	                    window.parent.document.getElementById($("#cursorDivId").val()).focus();
        	                }
        	            },
        	            {
        	                className:'edui-cancelbutton',
