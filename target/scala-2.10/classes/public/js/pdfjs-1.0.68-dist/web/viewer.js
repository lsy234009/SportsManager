...
'use strict';
//assets/js/pdfjs-1.0.68-dist/web/企业培训产品20140329.pdf`
var DEFAULT_URL = '';
...
PDFJS.workerSrc = '/assets/js/pdfjs-1.0.68-dist/build/pdf.worker.js';
  PDFJS.cMapUrl = '/assets/js/pdfjs-1.0.68-dist/web/cmaps/';
...
function getPDFFileNameFromURL(url) {
  ...
  var reFilename = /[^\/?#=]+\.pdf\b(?!.*\.pdf\b)/i|/[^\/?#=]+\.docx\b(?!.*\.docx\b)/i;
...
//window.addEventListener('resize', window.onload=function webViewerResize(evt) {
	//  (document.getElementById('pageFitOption').selected ||document.getElementById('pageWidthOption').selected ||document.getElementById('pageAutoOption').selected)
  //if (PDFView.initialized&&(document.getElementById('pageFitOption').selected ||document.getElementById('pageWidthOption').selected ||document.getElementById('pageAutoOption').selected)) {
...
window.addEventListener('change', function webViewerChange(evt) {
  ...
  // 上传文件
	...
	if(filetype.substring(0,filetype.indexOf("/"))!="image" && filetype.substring(0,filetype.indexOf("/"))!="audio" ){
		$.ajaxFileUpload({
			url : '/EntryQuestionsAction_savePdf/'+$("#examinationPaperId").val(), 
			async : false,
			secureuri : false,
			...
			success : function(data, status) {
				...
				var audioPath=data.url;
				DEFAULT_URL = "/returnPdf/" + audioPath.replace(/\//g,"^");
				PDFView.initialize().then(webViewerInitialized);
